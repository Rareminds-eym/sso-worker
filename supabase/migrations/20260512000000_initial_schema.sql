...
begin
  ...
  loop
    -- Drop existing deny_all policy if it exists, then recreate
    execute format(
      'drop policy if exists "deny_all_%s" on %I',
      tbl, tbl
    );
    execute format(
      'create policy "deny_all_%s" on %I for all to anon using (false) with check (false)',
      tbl, tbl
    );
  end loop;
end $$;
